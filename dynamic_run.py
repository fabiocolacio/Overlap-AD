...
if __name__ == "__main__":
	...
	while(i < cap_data): # Last 2 data will not be classified
		...
		all_max = max(max(t0_sub_dim[2]), max(t1_sub_dim[2]), min(t2_sub_dim[2]))
		...
		# Start codition
		if( (len(scaled_set_t1[0]) == len(scaled_set_t2[0]) or len(scaled_set_t1[0]) == len(scaled_set_t2[0])) or
			(CS_t0_LCE[0][t0_LCE] < CS_t1_LCE[0][t1_LCE]) or (CS_t1_LCE[0][t1_LCE] < CS_t2_LCE[0][t2_LCE])):
			result_list.append(-100)
